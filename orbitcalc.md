<img alt="" src="images/cover.png" width="1030" />
<br>
<div class="page"/>
<br>
...
今の技術でロケットに代わる安価な宇宙往還システムを考えてみます。演習編のように数式を提示したかったのですが、解析的に解けないところも多く、今回は検討した結果の紹介に留めました。
...
では、鉄筋コンクリートで作成した場合は、根元の大きさはどうなるでしょうか?
...
すごい値になりますね。桁が2桁異なります。



地表で$10\times10^-3$ 平方メートル すなわち断面積 1平方cm のテザー1本だけの構造で考えてみます。
これでも、102 トンを持ち上げることができます。静止軌道上では0.145平方メートルということになります。

テザーの全体重量は 1/1000 となり、20万トン〜30万トンとなります。
うーん、宇宙に30万トンを輸送!?

先にアンカーとして小惑星を挙げましたが、小惑星が無いとするとテザーを長く延ばしてその遠心力を案顔の代わりに使うことになります。先の論文の図を引用すると、
...
先の計算では、ロケットで運ぶのはかなり無理な質量でした。

ところで吊橋の建設でエアスピニングという工法があります。吊橋のワイヤを一度にかけるのではなく、まず細いワイヤを1本だけ通し、それを伝って2本目、3本目のワイヤをかけて補強していく方法です。
...
800kmの全長のテザーを高度400kmにおき、400秒で1回転してみます。
末端のGは10Gです。
...
![プローブアンドドローグ式の空中給油<span class=pin>[^11]</span>](image-16.png)
...
LEOは 100kmから2000km程度ですが、100km〜200km程度の超低高度の衛星軌道からなら地表にテザーを下ろせば破断長を超えずに地表に届くのではないでしょうか。

仮に鋼鉄ワイヤなら破断長は60kmなので60kmより下ろせば切れてしまいそうですね!

しかしながら鋼鉄ワイヤも衛星軌道と同様に軌道速度を得ていれば、重力は働かないので地表に降りることにななります。

しかしながらこれだと極超音速スカイフックと変わらないですね。軌道が低い分、ランデブー相対速度が大きくなってしまいます。
それを解消するために、非同期軌道スカイフックではテザーを自転させましたが、ここでは自転させずに考えてみます。
...
![軌道上のステーション](image-1.png)

テザーを「炭素繊維東レ T1100G」製、断面積1平方cmとするとテザー質量は179トンとなります。



これを、 このように地表に垂らしてみます。テザー先端は大気に吹き流されます。
...
テザーの最も先行している場所は、減速されたテザーの折り返し点です。その屈曲カーブで推進力が発生します。正確ではないのですが、この先端部、あるいは屈曲カーブを屈曲点と呼ぶことにします。テザー屈曲点の進行速度が4.3km/s の場合、2000kN 以上の推進力が屈曲点から得られます。この推進力に引っ張られてシャトルは加速していきます。
![シャトル概要](image-26.png)
...
そもそも、テザーは断面積1平方cm で想定しているので 700kN 程度で破断してしまいます。
...
「炭素繊維東レ T1100G」の破断長399kmですが、テザー先端部はシャトルといっしょに3G加速するので、その部分の破断長は133kmとなってしまいます。シャトルの 7t の質量がかかることを考えると、3G加速を行うテザー領域は94kmまでである必要があります。

しかしながら屈曲点よりも先端はどんどん長さが増大していきます。それも含めて3G加速を行わないといけません。更に屈曲点から引き出す推力も、長さが増大するに従い増やさないといけません。

これに対処するにはテザーをある程度からテーパーをつけることで、屈曲点の半径や強度を補うようにします。先の想定では、

実際には、屈曲した場所のテザーは増えていくため、3G加速は時間と共に低下します。


最初に示した条件では、テザーを「炭素繊維東レ T1100G」製、断面積1平方cmとしてテザー質量は179トンとしましたが、1000トン程度まで増やす必要があります。
...
![WINTLETT:WINch To LEo by Twenty cenury Technologies ](image-35.png)



![軌道ウインチステーション](image-43.png)
...
<img alt="picture 2" src="image-57.png" width="700" />
軌道上のステーションからテザーを撃ち下ろします。

![テザーは地上に近づくにつれて先行します。](image-61.png)

![滑走路から離陸](image-40.png)

地上の飛行場からシャトルが離陸します。

![軌道ウインチシャトル](image-46.png)
- 全備重量 7トン(ドッキング時)
- 離陸重量 8トン
- 最大積載量 2トン
- ターボジェットエンジンx2
...
![ペイロード搭載方法](image-34.png)


![テザーへの大気圏でのアプローチ](image-54.png)
![テザーをキャプチャ](image-47.png)
![テザーによる機動](image-45.png)
![宇宙へ到達](image-41.png)
...
あとがき
WINTLETT を構想してから10年、全然進まなかったのを今回この書籍にしてみました。厳密に式を立てていません。次回は式を使って、より厳密なものをしたいと思います。
それができたあとは、バリエーションを増やそうと思います。-- 


俺たちのたたかいはこれからだ！

2023.08.13 04:16  Nearer, My God, to Thee を聞きながら
...
<br>
<br>
<img alt="picture 2" src="image-57.png" width="538" />
